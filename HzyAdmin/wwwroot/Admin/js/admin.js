...
var admin = {
    ...
    alert: function (content, type, title) {
        switch (type) {
            case "警告":
                admin.layer.alert(content, { icon: 0, offset: "t", title: title || "警告" });
                break;
            case "成功":
                admin.layer.alert(content, { icon: 1, offset: "t", title: title || "成功" });
                break
            case "错误":
                admin.layer.alert(content, { icon: 2, offset: "t", title: title || "错误" });
                break;
            default:
                admin.layer.alert(content, { offset: "t", title: title || "提醒" });
        }
    },
    //提示框
    msg: function (content, type, time) {
        switch (type) {
            case "警告":
                admin.layer.msg(content, { icon: 0, offset: "t", time: time || 8 * 1000 });
                break;
            case "成功":
                admin.layer.msg(content, { icon: 1, offset: "t", time: time || 8 * 1000 });
                break
            case "错误":
                admin.layer.msg(content, { icon: 2, offset: "t", time: time || 8 * 1000 });
                break;
            default:
                admin.layer.msg(content, { offset: "t", time: time || 8 * 1000 });
        }

    },
    //询问框
    confirm: function (content, yes, cancel, title) {
        admin.layer.confirm(content, { offset: "t", title: title || "提醒" }, yes, cancel);
    },
    //加载
    loading: {
        index: 0,
    ...
    koMapping: function (json, obj, objName) {
        ...
        for (var prop in json) {
            if (obj.hasOwnProperty(prop)) {
                ...
                var value = json[prop];
                if (value) {
                    if ((typeof value == 'string') && value.constructor == String) value = value.replace(/\r/g, "\\r").replace(/\n/g, "\\n");
                    code += "'" + value + "'";
                }
